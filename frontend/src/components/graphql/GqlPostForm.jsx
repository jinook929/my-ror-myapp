...
import { fetchPost, createPost, updatePost } from "../../api/graphql";

export default function GqlPostForm() {
  const { id } = useParams();
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (isEdit) {
        ...
        navigate(`/graphql/posts/${id}`);
      } else {
        const post = await createPost({ title, content });
        navigate(`/graphql/posts/${post.id}`);
      }
    } catch (err) {
      setErrors(err);
    }
  ...
  return (
    <>
      <h1 className="text-3xl font-bold mb-6">
        {isEdit ? "Edit Post" : "New Post"} (GraphQL)
      </h1>
